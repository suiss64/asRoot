...
ETCDCTL_API=3 etcdctl --endpoints $(echo $endpoints) --cacert=/etc/ssl/etcd/ca.crt --key=/etc/ssl/etcd/system\:etcd-peer\:etcd-0.crc.testing.key --cert=/etc/ssl/etcd/system\:etcd-peer\:etcd-0.crc.testing.crt snapshot save /tmp/snapshots/snapshot.db && aws2 s3 --endpoint-url=http://10.128.1.24:4572 cp /tmp/snapshots/snapshot.db s3://$(echo $bucketName)/snapshot.db && rm -f /tmp/snapshots/snapshot.db && fileName=$(aws2 --endpoint-url=http://10.128.1.24:4572 s3 ls s3://$(echo $bucketName) --recursive | sort | head -1 | awk '{print $4;}') && backupControl
